...
impl<'a> LayoutInfo<'a> {
    ...
    pub fn write<W: SgxsWrite>(&self, writer: &mut W) -> Result<(), Error> {
        ...
        const TLS_SIZE: u64 = 0x2000; // 2 pages: 1 for variables; one for temp stack
        let nssa = 1u32;
        ...
        for i in 0..self.threads {
            ...
            writer.write_pages(Some(&mut &tls[..]), (TLS_SIZE as usize) / 0x1000, Some(tls_addr), secinfo)?;
            ...
            let tcs = Tcs {
                ...
                ofsbasgx: tls_addr,
                ogsbasgx: stack_tos,
                fslimit: 0x1fff, // 2 pages (-1)
                gslimit: 0x1fff, // 2 pages (-1)
                ..Tcs::default()
            };
