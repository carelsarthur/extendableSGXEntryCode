...
class Breakpoints:
    def setup(self, proj, simgr, layout):
        ...
        # Test concerning the stack pointer
        simgr.active[0].inspect.b(
            "reg_write",
            reg_write_offset=48,  # Offset of the stack pointer register
            when=angr.BP_AFTER,
            action=lambda s: self.detect_stackpointer_violations(simgr, s, layout))
        ...
        # Memory tracking
        # Essentially for performing taint analysis
        # Everything coming from outside is untrusted --> make the read value symbolic
        # Will notice if we ever were to jump to this value that it's untrusted!
        simgr.active[0].inspect.b(
            'mem_read',
            when=angr.BP_AFTER,
    ...
    def detect_read_violations(self, simgr, state, layout):
        ...
        if state.solver.satisfiable(extra_constraints=[
            ...
            print("Found a read violation!", state.inspect.mem_read_address)
            print("     out of allowed range!, read operation length: " , length)
            print("     allowed range: ", hex(allowed_range_begin), " --> ", hex(allowed_range_end))
            if not read_allowed:
                log.warning(
    ...
    def detect_stackpointer_violations(self, simgr, state, layout):
        new_sp = state.inspect.reg_write_expr
        assert new_sp is not None
        violation = None
        # NOTE: we use BP_AFTER --> we do not have to worry about "length" as in the other cases in this file, right?

        # Found this method at https://github.com/benquike/cheatsheets/blob/master/tools/research/angr.md
        if state.get_plugin('enclave').temporary_stack is True:
            # log.error("using temporary stack @ {}".format(state.regs.rip))
            allowed_range_begin = layout.stack_tos + layout.PAGE_SIZE
            allowed_range_end = layout.stack_tos + layout.PAGE_SIZE * 2
        else:
            # log.error("NOT using temporary stack @ {}".format(state.regs.rip))
            allowed_range_begin = layout.stack_start 
            allowed_range_end = layout.stack_tos

        # Will have different limits based on temp stack / real stack cases!
        if (state.solver.satisfiable(extra_constraints=[claripy.Or(new_sp < allowed_range_begin, new_sp > allowed_range_end)]) # rsp needs to be within the limits
          and state.solver.satisfiable(extra_constraints=[state.regs.rax != 0x4, state.regs.rsp != state.solver.BVS("init_" + "rsp", 64)])):  # Or: Upon exit (rax == 4), the original stack pointer has to be reset!
             log.error("Using wrong stack: {} @ {}".format(state.inspect.reg_write_expr, state.regs.rip))
             violation = (ViolationType.StackOverflow,
                          ViolationType.StackOverflow.to_msg(),
